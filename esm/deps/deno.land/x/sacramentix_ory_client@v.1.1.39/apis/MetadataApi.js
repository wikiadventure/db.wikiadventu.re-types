...
export class MetadataApiResponseProcessor {
    ...
    async getVersion(response) {
        ...
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body = ObjectSerializer.deserialize(ObjectSerializer.parse(await response.body.text(), contentType), "GetVersion200Response", "");
            return body;
        }
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
    async isAlive(response) {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body = ObjectSerializer.deserialize(ObjectSerializer.parse(await response.body.text(), contentType), "HealthStatus", "");
            return body;
        }
    ...
    async isReady(response) {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body = ObjectSerializer.deserialize(ObjectSerializer.parse(await response.body.text(), contentType), "IsReady200Response", "");
            return body;
        }
        if (isCodeInRange("503", response.httpStatusCode)) {
            const body = ObjectSerializer.deserialize(ObjectSerializer.parse(await response.body.text(), contentType), "IsReady503Response", "");
            throw new ApiException(response.httpStatusCode, "Ory Kratos is not yet ready to accept requests.", body, response.headers);
        }
        if (isCodeInRange("0", response.httpStatusCode)) {
            const body = ObjectSerializer.deserialize(ObjectSerializer.parse(await response.body.text(), contentType), "string", "");
            throw new ApiException(response.httpStatusCode, "Unexpected error", body, response.headers);
